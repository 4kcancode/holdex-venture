...
const skeleton = (urls: string) => `<?xml version="1.0" encoding="UTF-8"?><sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls}</sitemapindex>`
...
export const GET: RequestHandler = async ({ locals }) => {
    ...
    if (list == null) {
        ...
        let files = "";
        list.forEach(item => { files = files.concat(generateSitemapFile(item)); })
...
async function getBatchCursor(nextCursor: string | undefined, locals: App.Locals) {
    const response = await loadFeedForSitemapCursor(locals.apolloClient, {
