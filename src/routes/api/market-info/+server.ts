...
export const GET: RequestHandler = async ({ fetch, url }) => {
    ...
    const chartPromise = fetch(`${clientConfig.utilsApiUrl}/market-data/coins?url=${marketUrl}&source=chart`, {
        headers: {
            'x-holdex-authorization': `Bearer ${config.utilsApiKey}`
        }
    }).then(res => res.json());

    const infoPromise = fetch(`${clientConfig.utilsApiUrl}/market-data/coins?url=${marketUrl}&source=info`, {
        headers: {
            'x-holdex-authorization': `Bearer ${config.utilsApiKey}`
        }
    }).then(res => res.json());

    return Promise.all([chartPromise, infoPromise]).then(([chart, info]) => {
